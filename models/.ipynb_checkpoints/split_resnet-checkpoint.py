...
class BinarizeFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        x, = ctx.saved_tensors
        sigmoid_x = torch.sigmoid(10 * x)
        grad = 10 * sigmoid_x * (1 - sigmoid_x)
        return grad_output * grad
...
class ResNet(nn.Module):
    def __init__(self, cfg, builder, block, layers, base_width=64):
        ...
        self.binary_masks = nn.ParameterList()
        self.binary_mask_names = []
        for name, param in self.named_parameters():
            if 'weight' in name and 'bn' not in name and 'downsample' not in name:
                mask = nn.Parameter(torch.zeros_like(param))
                self.binary_masks.append(mask)
                self.binary_mask_names.append(name)
                #print(f"ResNet: Layer {name}, weight shape: {param.shape}, mask shape: {mask.shape}")

        if cfg.logger:
            cfg.logger.info(f"Binary mask layers: {self.binary_mask_names}")
            cfg.logger.info(f"Binary mask shapes: {[mask.shape for mask in self.binary_masks]}")
    ...
    def forward(self, x):
        ...
        for name, param in self.named_parameters():
            if 'weight' in name and 'bn' not in name and 'downsample' not in name:
                param.data = param * binarize(self.binary_masks[mask_idx])
                mask_idx += 1
