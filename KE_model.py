...
def ke_cls_train(cfg, model, generation):
    ...
    for epoch in range(cfg.start_epoch, end_epochs):
        ...
        if not cfg.no_wandb:
            wandb.log({
                'Epoch': epoch + (generation*end_epochs), 
        ...
        if cfg.eval_intermediate_tst > 0 and cfg.eval_tst and (epoch+1) % cfg.eval_intermediate_tst == 0:
            ...
            if not cfg.no_wandb:
                wandb.log({
                    'Epoch': epoch, 
                    'Test Acc1': last_tst_acc1, 
                    'Test Acc5': last_tst_acc5,
                    'Test Loss': last_tst_loss
                })
        else:
            last_tst_acc1 = 0
            last_tst_acc5 = 0
    ...
    col_names = [
        'generation', 'sparcity', 'last_val_acc1', 'last_val_acc5', 'best_val_acc1', 'best_val_acc5',
        'last_tst_acc1', 'last_tst_acc5', 'best_tst_acc1', 'best_tst_acc5', 'best_train_acc1',
        'best_train_acc5', 'train_loss', 'val_loss'
    ]
    arg_list = [
        generation, cfg.sparsity, last_val_acc1, last_val_acc5, best_val_acc1, best_val_acc5, 
        last_tst_acc1, last_tst_acc5, best_tst_acc1, best_tst_acc5, best_train_acc1, 
        best_train_acc5, train_loss, last_val_loss
    ]
...
def ke_cls_train_fish(cfg, model, generation, fisher_mat):
    ...
    if cfg.autos:
        ...
        fisher_mat = prune.autos_prune(1 - cfg.sparsity, autos_model_path=cfg.autos_model_path)

    return run_base_dir, fisher_mat, model

def adjust_fisher(fim, net, filter_importance_statistic='norm'):
    layerwise_fim_norm = np.zeros(19, dtype=float)  # 19 layers: 1 conv + 2*9 conv + 1 fc
    ...
    with torch.no_grad():
        for name, param in net.named_parameters():
            if ('weight' in name) and ('bn' not in name) and ('shortcut' not in name) and len(param.shape) > 1:
...
def ke_cls_eval_sparse(cfg, model, generation, ckpath, name):
    ...
    last_tst_acc1, last_tst_acc5, last_tst_loss = validate(dataset.val_loader, model, criterion, cfg, writer, 0)
    if not cfg.no_wandb:
        wandb.log({
            'Generation': generation, 
            'Test Acc1': last_tst_acc1, 
    ...
    print(name, last_tst_acc1, last_tst_acc5, last_tst_loss)

    # log results in a csv file
    col_names = ['generation', 'last_tst_acc1', 'last_tst_acc5', 'last_tst_loss']
    arg_list = [generation, last_tst_acc1, last_tst_acc5, last_tst_loss]
