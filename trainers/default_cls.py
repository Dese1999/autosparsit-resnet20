...
def train(train_loader, model, criterion, optimizer, epoch, cfg, writer, mask=None):
    ...
    for i, data in enumerate(train_loader):
        images, target = data[0].to(cfg.device), data[1].long().squeeze().to(cfg.device)
        ...
        if cfg.cs_kd:
            ...
            if cfg.use_noisy_logit:
                output = output + torch.normal(mean=0, std=1, size=output.shape).to(cfg.device)
        ...
        if mask is not None:
            for (name, param), mask_param in zip(model.named_parameters(), mask.parameters()):
                if param.grad is not None and 'weight' in name and 'bn' not in name and 'shortcut' not in name:
        ...
        if i % cfg.print_freq == 0 or i == num_batches-1:
            ...
            if writer is not None:
                writer.add_scalar("train/loss", losses.val, t)
            progress.display(i)
...
def validate(val_loader, model, criterion, args, writer, epoch):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            images, target = data[0].to(args.device), data[1].long().squeeze().to(args.device)
            output = model(images)
            loss = criterion(output, target)
            ...
            if i % args.print_freq == 0:
                if writer is not None:
                    writer.add_scalar("test/loss", losses.val, i + (epoch * len(val_loader)))
                progress.display(i)
