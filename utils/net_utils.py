...
def get_model(args):
    ...
    if args.arch == 'resnet20':
        ...
        model = resnet20(input_shape=(3, 32, 32), num_classes=10, dense_classifier=False, pretrained=False)
    else:
...
def bar_plot(dict_values, save_path):
    layers = [
        'conv', 
        'block0.conv1', 'block0.conv2', 'block1.conv1', 'block1.conv2', 'block2.conv1', 'block2.conv2',
        'block3.conv1', 'block3.conv2', 'block4.conv1', 'block4.conv2', 'block5.conv1', 'block5.conv2',
        'block6.conv1', 'block6.conv2', 'block7.conv1', 'block7.conv2', 'block8.conv1', 'block8.conv2',
        'fc'
    ]
    ...
    plt.bar(layers, values, color='maroon', width=0.4)
    plt.xlabel("Layers")
    plt.ylabel("Sum of FIM param")
    plt.title("Sum of importance weights in each layer (ResNet20)")
...
def extract_sparse_weights(cfg, net, mask):
    ...
    with torch.no_grad():
        if cfg.autos or cfg.snip:
            for (name, param), param_mask in zip(sparse_net.named_parameters(), mask.parameters()):
                if 'mask' not in name:
